...
async def parse_intent(
    ...
    if weather:
        ...
        context_parts.append(
            f"Погода: {w.get('temp', '?')}°C, відчувається як {w.get('feels_like', '?')}°C, "
            f"вітер {w.get('wind', '?')} км/год, дощ {w.get('rain_chance', '?')}%"
        )
...
BRIEFING_SYSTEM = """Ти — персональний помічник "Мій день". Тобі потрібно скласти короткий ранковий або вечірній брифінг.

Правила:
- Українською, тепло і природно, без канцеляризмів
- Починай з привітання ("Доброго ранку" / "Добрий вечір")
...
async def generate_briefing(
    ...
    """Generate morning/evening briefing text."""
    now = datetime.now()
    is_morning = now.hour < 14

    data_parts = [
        f"Час: {now.strftime('%H:%M')}, {'ранок' if is_morning else 'вечір'}",
    ]
