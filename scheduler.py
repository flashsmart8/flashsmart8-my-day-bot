...
async def _check_reminders():
    ...
    try:
        ...
        # Use Kyiv timezone explicitly (Railway servers run in UTC)
        now = datetime.now(KYIV_TZ)
        today = now.strftime("%Y-%m-%d")

        log.info("REMINDER CHECK at %s (today=%s)", now.strftime("%H:%M"), today)
        ...
        for eid, edata in events.items():
            ...
            event_done = edata.get("done")
            event_reminded = edata.get("reminded")

            if event_date != today:
                continue
            if event_done:
                log.info("Event %s: skipped (done)", eid)
                continue
            if event_reminded:
                log.info("Event %s: skipped (already reminded)", eid)
                continue
            ...
            try:
                ...
                log.info(
                    "Event %s: text=%r time=%s remind_before=%d diff_min=%d (window: %d < diff <= %d)",
                    eid, event_text, event_time, remind_before, diff_min,
                    remind_before - 5, remind_before
                )

                if (remind_before - 5) < diff_min <= remind_before:
                    log.info("SENDING REMINDER for event %s", eid)
                    await send_push(
                        title="Нагадування",
                    ...
                    log.info("Event %s: not in reminder window", eid)

            except Exception as e:
                log.warning("Failed to process event %s: %s", eid, e)

        log.info("REMINDER CHECK done: %d reminders sent", sent_count)
