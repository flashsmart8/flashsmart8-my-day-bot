...
async def get_weather(city: str = "Київ") -> dict:
    ...
    for i in range(min(7, len(daily_dates))):
        try:
            d = date.fromisoformat(daily_dates[i])
            label = "сьогодні" if i == 0 else ("завтра" if i == 1 else weekdays_uk[d.weekday()])
            forecast.append({
                "date": daily_dates[i],
                "label": label,
                "high": round(daily_max[i]) if i < len(daily_max) else None,
                "low": round(daily_min[i]) if i < len(daily_min) else None,
                "rain_chance": daily_rain[i] if i < len(daily_rain) else 0,
                "description": _weather_code_to_text(daily_codes[i]) if i < len(daily_codes) else "",
            })
        except Exception:
            pass
